...
int main(int argc, char **argv) {
    ...
    printf("TX %s%s -> %s:%d (tsys=%d tcomp=%d, sysid=%d compid=%d)\n",
           arm ? "ARM" : "DISARM", force ? " FORCE" : "", ip, port, target_sys, target_comp, sysid, compid);
    ...
    while (monotonic_seconds() < deadline) {
        ...
        for (ssize_t i = 0; i < n; ++i) {
            ...
            printf("ACK cmd=%u %s(%u) rp2=%d progress=%u tsys=%u tcomp=%u\n",
                   (unsigned)ack.command, ack_result_str(ack.result), (unsigned)ack.result,
