...
#define MAV_SYSTEM_ENV_PATH "/etc/rocket-mav/ports.env"
#define MAV_USER_ENV_REL ".config/rocket-mav/ports.env"
...
static inline void mav_cfg_load_once(void) {
    ...
    mav_cfg_load_file(MAV_SYSTEM_ENV_PATH, state->sys_table, &state->sys_count);

    const char *home = getenv("HOME");
    if (home && home[0]) {
        char user_path[PATH_MAX];
        snprintf(user_path, sizeof(user_path), "%s/%s", home, MAV_USER_ENV_REL);
